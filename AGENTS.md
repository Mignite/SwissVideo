...
- 2026-08-20 — Cola/audio: badges de corte+audio en la misma fila; rework de restauración de audio eliminando el global `PendingAudioRestoreTracks` (carrera con video_info async) → `UpdateAudioTracks(Info, savedTracks)` lee el ítem por `CurrentVideoPath` en el handler. `ProbeQueueItemAudio`: al agregar un ítem a la cola se sondea su audio en background y el badge muestra el nombre real de la pista (p. ej. labels de OBS como "Todo"); sin audio → `[]` = "Sin audio". Fix doble-click en X del primer ítem: `ClearCurrentVideo()` no re-renderizaba la cola → ahora llama `RenderQueueList()`.
- 2026-08-20 — Backend anti-crash audio: `-map 0:N?` (sufijo opcional de ffmpeg) para pistas específicas; si la pista no existe, se ignora en vez de abortar el encode.
- 2026-08-20 — Rediseño selector de codec (mockups en `design/codec-selector.html`, elegido: diseño A + dropdown "Otros"): segmentado de familias [H.264|H.265|AV1|Otros▾] + fila Motor [CPU|GPU]. Botones `<button>` reales con focus-visible (a11y). Bugfixes incluidos: CodecMap `"libvpx-vp9"`→`libvpx_vp9` ({codec} en plantilla de nombre daba "h265"), etiquetas "GPU·NV/AMD/INT"→"· GPU" (marca única por sesión), guard de presets vía `AvailableCodecs` set. Token nuevo `--Mono`. Fallback: si SelectedCodec no está disponible, cae a la primera familia principal.
- 2026-08-20 — Menú "Otros" = catálogo completo: backend expone `video_encoders: Vec<String>` en FfmpegCapabilities (parsea TODOS los encoders de video de `ffmpeg -encoders`, excluye leyendas con "=", orden alfabético; ~140 en build típico). Frontend lista en Otros todos menos los del segmentado, con scroll (max-height 260px). Fallback de `{codec}` en plantilla de nombre ahora usa el nombre del encoder en vez de "h265". Nota: VP9 aparece como `libvpx-vp9` (nombre real de ffmpeg, el viejo `libvpx_vp9` con guion bajo quedaba cojo).
